...
if __name__=='__main__':
    ...
    PROJECT_NEWDIR = '/home/shuang/projects/boxy_eqtm'

    # gene position
    tss_raw_path = os.path.join(PROJECT_DIR,'data','TSSdistance',
                                'Homo_sapiens.GRCh37.71.gtf')

    # eqtmZscore file path
    eqtmZscore_folder = os.path.join(PROJECT_NEWDIR,'data','eqtmZscores')
    eqtmZscore_path = os.path.join(eqtmZscore_folder,
                                   'fdr_gt0.05','random20k_gt0.5.txt')
    eqtmZscoreTss_savepath = os.path.join(eqtmZscore_folder,
                                   'fdr_gt0.05','random20k_gt0.5_withZscore.txt')
    _ = addTssDistance2eQTMwithZscoreFile(eqtmZscore_path,
                                      tss_raw_path,
                                      eqtmZscoreTss_savepath)

    # add to overlapRatio the zscore and Tss
    overlapRatio_path = os.path.join(PROJECT_DIR,'data','output',
                                     'random20k_gt0.5_overlapRatio.txt')
    _ = changeCpGColumnName_addZscoreTss_forRandom(overlapRatio_path,
                                               '\t',eqtmZscoreTss_savepath,',')
    # add mean Variance
    overlapRatioTss_folder = os.path.join(PROJECT_NEWDIR,'data',
    ...
    meanVar_filepath = os.path.join(PROJECT_NEWDIR,'data',
                                    'features','meanVar',
                                    'methylation-MeanAndVarianceRows.txt')
    overlapRatioTss_filename = 'random20k_gt0.5_withZscoreTss'
