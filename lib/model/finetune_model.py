...
def examineModel(model_name,trainPath,testPath,iteration=4,savefilename=None,
    ...
    for i in range(iteration):
        ...
        assert "OverallZScore" not in train_data.values.columns
        ...
        if trainPath != testPath:
            test = load_data(testPath,
                             ...
                             exclude=exclude,
                             test_size=0)
            test_data = dataset(test.train.values[features_to_use],
                                test.train.labels)
            sensitivity2, specificity2, auc2 = \
            model.calculate_output(model.classifier,
                                   test_data)
...
def choose_model(model_name, num_features, num_samples):
    if model_name=='ranfor':
        ...
        # param_grid = {"ranfor__n_estimators": np.arange(2, 300, 100),
        #               "ranfor__max_depth": np.arange(1, 28, 7),
        #               "ranfor__min_samples_split": np.arange(0.1, 1, 0.5),
        #               "ranfor__min_samples_leaf": np.arange(1, 60, 30),
        #               "ranfor__max_leaf_nodes": np.arange(2, 60, 30),
        #               "ranfor__min_weight_fraction_leaf": np.arange(0.1, 0.4, 0.2),
        #               "ranfor__class_weight": class_weight}
        pipeline = Pipeline(steps=[('ranfor', RandomForestClassifier())])
