...
if __name__=='__main__':
    ...
    feature_type = 'imputedGappedPeak'
    ...
    cpg_path = os.path.join(PROJECT_DIR,'data','eqtmZscores','ORIGIN',
                            cpg_name+'.txt')
    save_path = os.path.join(PROJECT_DIR,'data','eqtmZscores','ORIGIN',
                             cpg_name+'_overlapMatrix.txt')
    ratio_table_savepath = os.path.join(PROJECT_DIR,'data','eqtmZscores','ORIGIN',
    ...
    names = ['chr_feat','str_feat','end_feat', 'type','chr_cpg','str_cpg','end_cpg','cpg_name']

    # start processing
    print('Process features in folder: ',data_folder)
    record_path = os.path.join(PROJECT_DIR,'commands','record',cpg_name+'.txt')
    ...
    for filename in os.listdir(data_folder):
        ...
        if os.stat(filepath).st_size == 0:
            ...
            row_index = [overlapTable.index.get_loc(i) for i in bedtoolsRes['cpg_name'].unique()]
