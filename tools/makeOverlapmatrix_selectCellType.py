...
if __name__ == '__main__':
    ...
    ratio_table_complete_savepath = args.savepath
    ...
    # read blood-only or non_blood celltype

    #for non-blood cells or blood-only cells
    with open(os.path.join("/groups/umcg-gcc/tmp03/umcg-sli/development_eqtm/data/features/celltype/roadmap-celltypes-blood-MJ.txt"),
              "r") as f:
        blood_celltypes = [item.strip() for item in f.readlines()]
    print("Blood celltypes", len(blood_celltypes))
    non_blood_cells = [cell for cell in cell_type_list if cell not in blood_celltypes]
    print("Non-blood celltypes", len(non_blood_cells))

    celltype = args.celltype

    if celltype == 'blood':
        print("Using blood cell types.")
        chosen_cells_set = set(blood_celltypes)
    elif celltype == 'non_blood':
        print("Using non-blood cell types")
        chosen_cells_set = set(non_blood_cells)
    else:
        print("Using all cell types.")
        chosen_cells_set = set(cell_type_list)
    ...
    for filename in os.listdir(data_folder):
        ...
        if os.stat(filepath).st_size == 0:
            ...
            bedtoolsRes = bedtoolsRes.dropna()
            ...
            cell_name = feature_name.split("-")[0]
            if cell_name in chosen_cells_set:
                ratio_data[row_index, histone_index] += 1.0/len(chosen_cells_set)
    ...
    ratio_table_savepath = args.savepath
    ratio_dataframe.to_csv(ratio_table_savepath, sep="\t")
    print("Saved in ", ratio_table_savepath)
