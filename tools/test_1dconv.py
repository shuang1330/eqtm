...
def order_histone(histone_list, dic):
    corr_index = np.argsort(np.array([dic[histone] for histone in histone_list]))
    return np.array(histone_list)[corr_index]
...
if __name__ == "__main__":
    ...
    histone_list_filepath = os.path.join(project_rootdir,
                                         ...
                                         "meta",
                                         "histone_list.txt")
    spear_cor_filepath = os.path.join(project_rootdir,
                                      "data",
                                      "features",
                                      "feature_importances",
                                      "spearcor.txt")
    ...
    with open(spear_cor_filepath, "r") as f:
        ...
        for line in content:
            ...
            feature_cor = line.split("\t")[1]
            spear_dic[feature_name] = feature_cor
        spear_feature = [item.split("\t")[0] for item in content]
        spear_cor = [item.split("\t")[1] for item in content]
        f.close()
    print(spear_feature)
    print(spear_cor)
    ...
    ordered_histone_list = order_histone(histone_list, spear_dic)
    print(histone_list)
    print("After ordering:")
    print(ordered_histone_list)
    test_features = np.array(data.test.values[ordered_histone_list].values)
